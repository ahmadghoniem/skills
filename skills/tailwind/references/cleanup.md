...
- **Collapse same-value pairs — order is not part of the pattern.** `px-N py-N` → `p-N`; `mx-N my-N` → `m-N`; all four `pt/pb/pl/pr` → `p-N`; `w-N h-N` → `size-N`. `h-5 w-5` collapses exactly like `w-5 h-5` — match the pair as a **set**, or let the linter do it.
- **Axis pairs too.** `top-N bottom-N` → `inset-y-N`; `left-N right-N` → `inset-x-N`; all four → `inset-N`. Same shape for `border-x/y`, `scroll-m*`, `scroll-p*`, `divide-x/y`.
...
**Colour format**
- hex / `rgb()` / `hsl()` in `:root`, `.dark`, or `@theme` → convert to `oklch()`. Convert **values only** — leave `currentColor`, CSS keywords, gradient interpolation, and third-party library configs alone. Never compute by hand: `node ~/.claude/skills/tailwind/scripts/oklch.mjs --table '#0f172a' '#f5f5f4'` emits the Before/After rows directly.
...
- In a component framework this is a missing **component**, not a missing CSS class — flag it as that. `@utility` here just moves the duplication into a file the reader has to go find.
- `@utility` is right only where no component can own the markup (CMS output, a third-party widget, a design system used from two frameworks), or for an existing `@layer components` block. Read `references/affordances.md` first. Never for a string that appears once.
...
No Tailwind linter parses these, so this part of the pass is manual. Grep the same files for `#[0-9a-f]{3,8}`, `rgb(`, `hsl(`, `style={{`.

- A colour in a `style={{}}` or a `.ts` constant is a violation wherever it appears — an inline literal cannot flip under `.dark`. Flag every one.
- **Only runtime-computed properties stay inline** (a `top` from `getBoundingClientRect()`, a state-driven `transform`). Every *static* key in the same object moves out down `SKILL.md`'s ladder: `borderRadius: "2px"` → `rounded-xs`, not `rounded-[2px]`. Split the object; don't wave it through because some of its keys are computed.
- `fill=` / `stroke=` on inline SVG → `currentColor` plus a text utility, unless deliberately multi-colour.

Report these in the colour Before/After table with the rest.
