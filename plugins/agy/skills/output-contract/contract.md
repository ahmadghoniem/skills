...
| :--- | :--- | :--- |
| `agy-status` | `⚠ agy status: <status> (write-up present, N files changed)` | agy's own verdict, verbatim. Not a pass/fail — agy reports `ERROR` on runs that worked and `SUCCESS` on runs that did not. The parenthetical is there because a bare `ERROR` reads identically for a provider blip in the last second of a complete run and for a run that died having written nothing. Both facts in it are measured, not judged: whether a write-up exists, and the file count from two `git status --porcelain` snapshots taken either side of the run. The file count is omitted outside a git repo. It disambiguates the status; it does not decide it. |
...
| `resume` | `⚠ this run can be resumed where it stopped: /agy:resume <id>` | Only fires alongside the watchdog line, and only when the conversation id was captured — which it always is on a run that got as far as being killed, since `conversation_id` arrives on the `init` event. Says the option exists; does not tell you to take it. Re-dispatching the whole brief instead is a legitimate choice, and the right one when the run had gone off the rails before the timeout. |
