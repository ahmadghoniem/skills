...
| `resumable` | `⚠ this run ended early — resume it with /grok:resume --resume=<job-id>` | The other half of the line above, and mutually exclusive with it: the run was cut short but an id exists to attach to, which is the usual shape of a watchdog kill since a fresh dispatch pre-assigns its session with `-s`. Says the option exists; does not tell you to take it. Re-delegating is still the right call when the run had gone off the rails before it was killed. |
