...
| `resumable` | `⚠ this run did not finish cleanly — resume it with /grok:resume --resume=<job-id>` | The other half of the line above, and mutually exclusive with it: the run ended short of a clean finish — killed, a non-zero exit, a stop reason short of `end_turn` — but a session exists to attach to, which is the usual shape since a fresh dispatch pre-assigns its session with `-s`. The id is the job id, so the line can be pasted as printed. Says the option exists; does not tell you to take it. Re-delegating is still the right call when the run had gone off the rails before it stopped. |
