...
the record, not a precis of it. Lines starting `⚠` are the plugin's warnings; keep
every one, and keep them separate. `⚠ stop reason: …` (grok's own verdict) and
`⚠ exit N` (the process exit code) are independent facts that disagree in both
directions, and `⚠ N commands exited non-zero` is the part most likely to
contradict grok's own summary.
