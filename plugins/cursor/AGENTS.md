...
5. **Jobs are persisted under `~/.ccd/jobs/<repo-hash>/`** (override with `CCD_HOME`). Never break that layout; users point scripts at those files when reporting bugs.
...
5. Open a PR against `main` with a summary + test plan. CI must pass across Node 18.18 / 20 / 22 × Windows / Linux.
6. Squash-merge only.
...
- `plugins/cursor/scripts/<cmd>.mjs` — command entrypoints.
